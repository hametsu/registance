		...
		setInterval(function(){
			...
			for (var i = 0,max = resent_log.length;i < max; i++){
				...
				//echo "<li style='color:".$log_array[2]."'><span class='name'>".$log_array[0].":</span>".$log_array[3]."</li>";
					$("<li/>").css("color",resent_log[i]["color"]).css("display","hiddden").html("<span class='name'>" + resent_log[i]["name"] + ":</span>" + resent_log[i]["message"] + " :: This Time is " + reflesh_time + " :: Time = " + resent_log[i]["time"]).fadeIn("slow").prependTo("#show_log");
					break;
				case "warning":
